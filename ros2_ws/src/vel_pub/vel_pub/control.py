...
class XboxController(object):
    ...
    def __init__(self, deadzone=0.1, retries=5, stall=5, atloss=lambda: [], empty=False):
...
class ControlPublisher(Node):
    ...
    def timer_callback(self):
        twist_msg = Twist()
        twist_msg.linear.x = float(controller.RT-controller.LT)*5
        twist_msg.angular.z = float(controller.RJoyX)*2
